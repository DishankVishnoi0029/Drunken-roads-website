...
const AllUsers = () => {
  ...
  const getUsers = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${import.meta.env.VITE_URL}/api/user/getAllUsers?searchTerm=${search}`);
  ...
  const handleUserDelete = async (userId) => {
    ...
    if (CONFIRM) {
      setLoading(true);
      try {
        const res = await fetch(`${import.meta.env.VITE_URL}/api/user/delete-user/${userId}`, {
          method: "DELETE",
        });
        const data = await res.json();
