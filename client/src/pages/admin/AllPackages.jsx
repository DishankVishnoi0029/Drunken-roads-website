...
const AllPackages = () => {
  ...
  const getPackages = async () => {
    ...
    try {
      ...
      let url =
        filter === "offer" //offer
          ? `${import.meta.env.VITE_URL}/api/package/get-packages?searchTerm=${search}&offer=true`
          : filter === "latest" //latest
          ? `${import.meta.env.VITE_URL}/api/package/get-packages?searchTerm=${search}&sort=createdAt`
          : filter === "top" //top rated
          ? `${import.meta.env.VITE_URL}/api/package/get-packages?searchTerm=${search}&sort=packageRating`
          : `${import.meta.env.VITE_URL}/api/package/get-packages?searchTerm=${search}`; //all
  ...
  const handleDelete = async (packageId) => {
    try {
      setLoading(true);
      const res = await fetch(`${import.meta.env.VITE_URL}/api/package/delete-package/${packageId}`, {
        method: "DELETE",
      });
      const data = await res.json();
      alert(data?.message);
