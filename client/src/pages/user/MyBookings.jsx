...
const MyBookings = () => {
  ...
  const getAllBookings = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_URL}/api/booking/get-UserCurrentBookings/${currentUser?._id}?searchTerm=${searchTerm}`
      );
  ...
  const handleCancel = async (id) => {
    try {
      setLoading(true);
      const res = await fetch(
        `${import.meta.env.VITE_URL}/api/booking/cancel-booking/${id}/${currentUser._id}`,
        {
          method: "POST",
        }
      );
