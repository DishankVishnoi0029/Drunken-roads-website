...
const MyHistory = () => {
  ...
  const getAllBookings = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_URL}/api/booking/get-allUserBookings/${currentUser?._id}?searchTerm=${search}`
      );
  ...
  const handleHistoryDelete = async (id) => {
    try {
      setLoading(true);
      const res = await fetch(
        `${import.meta.env.VITE_URL}/api/booking/delete-booking-history/${id}/${currentUser._id}`,
        {
          method: "DELETE",
        }
      );
